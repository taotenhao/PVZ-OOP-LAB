...
public class Conehead extends NormalZombie {
    ...
    static {
        try {
            walk1 = ImageIO.read(new File("Graphic/Conehead_1__walk.png"));
            w1W = walk1.getWidth() / 7;
            w1H = walk1.getHeight();
        } catch (Exception e) { e.printStackTrace(); }
        
        try {
            eat1 = ImageIO.read(new File("Graphic/Conhead_1__eat.png")); // Or Conehead_1__eat.png if user fixes it
            e1W = eat1.getWidth() / 7;
            e1H = eat1.getHeight();
        } catch (Exception e) {
            try {
                eat1 = ImageIO.read(new File("Graphic/Conehead_1__eat.png"));
                e1W = eat1.getWidth() / 7;
                e1H = eat1.getHeight();
        ...
        try {
            walk2 = ImageIO.read(new File("Graphic/Conehead_2__walk.png"));
            w2W = walk2.getWidth() / 7;
            w2H = walk2.getHeight();
        } catch (Exception e) { e.printStackTrace(); }

        try {
            eat2 = ImageIO.read(new File("Graphic/Conehead_2__eat.png"));
            e2W = eat2.getWidth() / 7;
            e2H = eat2.getHeight();
        } catch (Exception e) { e.printStackTrace(); }

        try {
            walk3 = ImageIO.read(new File("Graphic/Conehead_3__walk.png"));
            w3W = walk3.getWidth() / 7;
            w3H = walk3.getHeight();
        } catch (Exception e) { e.printStackTrace(); }

        try {
            eat3 = ImageIO.read(new File("Graphic/Conehead_3__eat.png"));
            e3W = eat3.getWidth() / 7;
            e3H = eat3.getHeight();
        } catch (Exception e) { e.printStackTrace(); }
    }

    public Conehead(String path, int x, int y) {
        super(path, x, y);
        this.health = 240;
    }

    private boolean coneFell = false;
    public boolean hasConeFell() { return coneFell; }
    public void markConeFell() { coneFell = true; }
